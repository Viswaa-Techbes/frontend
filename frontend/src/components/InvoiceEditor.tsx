...
export default function InvoiceEditor({ mode, documentId }: InvoiceEditorProps) {
  ...
  const getDraftKey = (businessIdStr: string) => {
    ...
    return `techbes_invoice_draft_${businessIdStr || 'default'}_${docIdStr}`;
  };
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      let backendDocState: any = null;
      if (mode === 'edit' && documentId) {
        const docRes = await api.get(`/documents/${documentId}`);
        if (docRes.data?.success && docRes.data.data) {
          backendDocState = docRes.data.data;
          setCleanBackendState(backendDocState);
          applyDocDataToState(backendDocState);
        }
      }

      // 4. Check for localStorage recovery state
      if (typeof window !== 'undefined') {
        const draftKey = getDraftKey(currentBizId);
        const storedDraftStr = localStorage.getItem(draftKey);
        if (storedDraftStr) {
          const stored = JSON.parse(storedDraftStr);
          // Show recovery banner if:
          // - create mode: always show if stored draft exists
          // - edit mode: if stored draft's timestamp is newer than document's updatedAt
          const backendTime = backendDocState?.updatedAt ? new Date(backendDocState.updatedAt).getTime() : 0;
          const storedTime = stored.timestamp || 0;
      ...
      showToast(err.response?.data?.message || 'Error loading initialization data.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const applyRecoveredState = (data: any) => {
    ...
    setPlaceOfSupply(data.placeOfSupply || { state: '', stateCode: '' });
    setReverseCharge(data.reverseCharge ?? false);
    setTaxType(data.taxType || 'Auto');

    setEnableShipping(data.enableShipping || false);
    ...
    setNumberFormat(data.numberFormat || 'en-IN');

    setItems(data.items || []);
    setEnableDocDiscount(data.enableDocDiscount || false);
    setDocDiscountType(data.docDiscountType || 'PERCENTAGE');
    setDocDiscountValue(data.docDiscountValue || 0);
    setAdditionalCharges(data.additionalCharges || []);
    setSummarizeTotalQuantity(data.summarizeTotalQuantity || false);

    setSignatoryName(data.signatoryName || '');
    setSignatoryDesignation(data.signatoryDesignation || '');
    ...
    setShowAdditionalInfo(!!data.additionalInfo?.length);

    setContactDetails(data.contactDetails || { name: '', phone: '', email: '' });
    setShowContactDetails(!!data.contactDetails?.name);

    setIsRecurring(data.isRecurring || false);
    setRecurrenceFreq(data.recurrenceFreq || 'Monthly');
    setRecurrenceStart(data.recurrenceStart || '');
    setRecurrenceEnd(data.recurrenceEnd || '');
    setRecurrenceInterval(data.recurrenceInterval || 1);
    setRecurrenceIntervalUnit(data.recurrenceIntervalUnit || 'Months');

    setDisplayOptions(data.displayOptions || displayOptions);
    setColumnVisibility(data.columnVisibility || columnVisibility);
  };

  // Apply database values
  const applyDocDataToState = (doc: any) => {
  ...
  // Discard recovery changes and re-load backend values
  const handleDiscardRecovery = () => {
    if (typeof window !== 'undefined' && businessProfile) {
      const draftKey = getDraftKey(businessProfile._id);
      localStorage.removeItem(draftKey);
    }
    if (mode === 'create') {
      // Re-initialize to default blank values
      setDocTitle('Invoice');
      setDocSubtitle('');
    ...
    } else if (cleanBackendState) {
      applyDocDataToState(cleanBackendState);
    }
  ...
  useEffect(() => {
    ...
    docTitle, docSubtitle, poNumber, issueDate, dueDate, customFields,
    selectedClientId, gstEnabled, placeOfSupply, reverseCharge, taxType,
  ...
  const handleSaveDocument = async (draftOnly: boolean, redirectSetup: boolean) => {
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        // Clear local draft recovery state upon save
        if (typeof window !== 'undefined') {
          const draftKey = getDraftKey(businessProfile._id);
          localStorage.removeItem(draftKey);
        }
  ...
  return (
    <div className="space-y-6 text-slate-800 pb-16">
      ...
      {showRecoveryBanner && (
        <div className="bg-indigo-50 border border-indigo-200 text-indigo-800 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs shadow-sm animate-fadeIn">
      ...
      {businessProfileIncomplete && (
        <div className="bg-amber-50 border border-amber-200 text-amber-800 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs shadow-sm">
          <div className="flex items-center gap-2">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          ...
          <h1 className="text-xl font-bold text-slate-900 mt-1">
            {mode === 'create' ? 'Create New Invoice' : `Edit Invoice ${documentNumber}`}
          </h1>
        </div>
        ...
        <div className="xl:col-span-3 card-panel p-6 sm:p-8 rounded-2xl bg-white shadow-sm border border-slate-200 space-y-8">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                  <h3 className="text-[10px] font-bold text-slate-455 tracking-wider uppercase">Billed By</h3>
                  <Link href="/settings" className="text-[10px] font-bold text-blue-600 hover:text-blue-700">Edit Details</Link>
                </div>
                <div className="text-xs text-slate-650 space-y-1 bg-slate-50/50 p-3 rounded-xl border border-slate-100">
          ...
          <div className="border-t border-slate-100 pt-6 space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Billed To</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
              ...
              <div className="md:col-span-2">
                {selectedClient ? (
                  <div className="text-xs text-slate-600 space-y-1 bg-slate-50/50 p-4 rounded-xl border border-slate-100 animate-slideUp">
          ...
          <div className="border-t border-slate-100 pt-6 grid grid-cols-1 md:grid-cols-3 gap-6 text-xs bg-slate-50/50 p-4 rounded-2xl border border-slate-200">
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">GST Mode</label>
          ...
          <div className="border-t border-slate-100 pt-6 grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
            <div className="space-y-6">
              <div>
                <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-3">Optional Invoice Details</h3>
              ...
              {showSignatureArea && (
                <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl space-y-4 animate-slideUp">
                  <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Authorized Signatory</h4>
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Signature image</label>
                    {signatureBase64 ? (
          ...
          <div className="border-t border-slate-100 pt-6 space-y-4">
            <label className="flex items-center gap-2 cursor-pointer select-none">
              ...
              />
              <div>
                <span className="text-xs font-semibold text-slate-700 block">This is a Recurring invoice</span>
      ...
      {isColumnDrawerOpen && (
        ...
        >
          <div className="space-y-4">
            <p className="text-xs text-slate-500">Enable/disable visible columns inside the invoice line items editor table.</p>
            <div className="grid grid-cols-2 gap-3 text-xs">
          ...
          <form onSubmit={handleCreateClient} className="space-y-4">
            ...
            <div className="space-y-3 border-t border-slate-800/80 pt-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                ...
                <div>
                  <label className="block text-[9px] font-bold text-slate-455 uppercase tracking-wider mb-1">State</label>
                  <select
