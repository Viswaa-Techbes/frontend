...
export default function SalesOrderEditor({ initialId }: SalesOrderEditorProps) {
  ...
  const [validTill, setValidTill] = useState(new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().substring(0, 10));
  ...
  // New inline client creation states
  ...
  // Fetch initial profile & sequencers
  ...
  const getGstTax = () => {
    // Calculates simple tax splits for preview purposes
      ...
      if (res.data?.success && res.data.data?._id) {
        showToast(
          targetStatus === 'DRAFT'
            ...
            : 'Sales Order finalized and saved!',
          'success'
        );
      ...
      <div className="border-b border-slate-200 pb-4 flex justify-between items-center">
        <div>
          <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">
            Dashboard / <Link href="/sales-orders" className="hover:underline">Sales Orders</Link> / New
          </span>
          <h1 className="text-xl font-bold text-slate-909 mt-1">
            {initialId ? `Edit Sales Order ${documentNumber}` : 'Create New Sales Order'}
          </h1>
        </div>
      </div>
        ...
        <div className="xl:col-span-3 space-y-6">
          ...
          <div className="card-panel p-8 sm:p-12 rounded-2xl bg-white shadow-sm border border-slate-200 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Sales Order No *</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">PO Number (optional)</label>
                <input
                  type="text"
                  value={poNumber}
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Valid Till</label>
                  <input
                    type="date"
                    value={validTill}
            ...
            {/* Billed From vs Billed For address blocks */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border-t border-slate-100 pt-6">
              {/* Business Profile details */}
              <div className="space-y-2 text-xs">
                <span className="block font-bold text-[9px] text-slate-400 uppercase tracking-wider">SALES ORDER FROM (YOUR DETAILS)</span>
              ...
              <div className="space-y-2 text-xs relative">
                <span className="block font-bold text-[9px] text-slate-400 uppercase tracking-wider">SALES ORDER FOR (CLIENT'S DETAILS)</span>
                {selectedClient ? (
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-3">
              <button
                onClick={() => handleSaveSalesOrder('ISSUED', true)}
                disabled={loading}
                className="px-5 py-2.5 border border-slate-355 bg-white hover:bg-slate-50 font-bold rounded-xl text-slate-705 transition-colors disabled:opacity-50 text-xs"
              >
                Save & Create New
              ...
              <button
                onClick={() => handleSaveSalesOrder('ISSUED')}
                disabled={loading}
